...
PRIVATE gchelp
gchelp = "Activities and Services Report Screen"
ccTitle = "Activities And Services Report"
...
SELECT ;
	...
	SPACE(2) AS how_prov,SPACE(5) AS worker_id,;
...
* jss, 3/6/03, add field 21 (serv_id) and field 22 (topic) to ordering of cursor temp
* jss, 3/7/03, add new topic-filled cursor MyServ3 to union
SELECT ;
	collat.*, ;
	SPACE(50) AS topic ;
FROM ;
...
**ORDER BY 17, 21, 22		&& list, serv_id, topic
...
** jss, 2/7/01, change ngroup from 1 to 2 in select statement below and ensuing similar selects below that
* jss, 10/98, next 4 select statements give report totals for clients,encounters,services, and items on services (respectively)
...
* jss, 3/7/03, add casopen clause below for consistency
...
* jss, 3/7/03, add report totals for topics here
...
* jss, 10/98, next 3 select statements give program totals for clients,encounters, and services (respectively)
Select program ,;
...
* jss, 2/7/01, since "Active" is now the 2nd in array, change ngroup from 1 to 2 below
...
Do Case
   Case lnStat = 1    &&Activities and Services
     ...
     Else
        SELECT m.*, p.descript ;
        FROM myenc as m LEFT OUTER JOIN program as p on p.prog_id = m.program ;
        INTO CURSOR temp
                         
        SELECT t.*, c.serv, c.topic, c.prov, c.work, c.start, c.end, c.numitems, ;
          c.tottime, c.list, c.cheader, c.colname, c.servnote, b.descript as bill_to , ;
          s.servtot, e.enctot, l.clitot, p.toptot, i.totitems, ;
          gcagencyname as agencyname, oApp.gcversion as version, DTOC(oApp.gdverdate) as verdate, ;
          repcli.clitot as tot_clnts, repenc.enctot as tot_enctrs, repserv.servtot as tot_serv,;
          repitm.totitems as tot_items, reptop.toptot as tot_topics ;
        FROM temp as t LEFT OUTER JOIN collserv as c ON c.act_id = t.act_id ;
        LEFT OUTER JOIN bill_to as b ON b.serv_cat+code = t.serv_cat+t.bill_to ;
        LEFT OUTER JOIN toptot as p ON p.program = t.program ;
        LEFT OUTER JOIN itmtot as i ON i.program = t.program ;
        LEFT OUTER JOIN servtot as s ON s.program = t.program ;
        LEFT OUTER JOIN enctot as e ON e.program = t.program;
        LEFT OUTER JOIN clitot as l ON l.program = t.program ;
        INTO cursor tmp readwrite

        REPLACE toptot WITH 0 FOR ISNULL(toptot)
        DO CASE
           CASE nOrder = 1
              SELECT tmp
              INDEX on program+last_name+first_name+serv_cat+DTOS(act_dt) TAG repord

           CASE nOrder = 2
              SELECT tmp
              INDEX on program+ENCWORK+last_name+first_name+serv_cat+DTOS(act_dt) TAG repord

           CASE nOrder = 3
              SELECT tmp
            INDEX on program+ENCNAME+last_name+first_name+serv_cat+DTOS(act_dt)tag repod
        ENDCASE   

        SELECT tmp
        COPY to crRptpath+"Activities_services.dbf"

        oApp.display_crystal_reports("activities_services_by_program.rpt")
...
*!*           LcAction = "open" 
*!*           Lcparms = "activities_services_by_program.rpt"
   ...
   Case lnStat = 2   &&Activities and Services by Site
     ...
     Else
        ...
        DO CASE
           CASE nOrder = 1
             SELECT tmp
             INDEX on sitename+program+last_name+first_name+serv_cat+DTOS(act_dt) TAG repord
             
           CASE nOrder = 2
             SELECT tmp
             INDEX on sitename+program+ENCWORK+last_name+first_name+serv_cat+DTOS(act_dt) TAG repord
             
           CASE nOrder = 3
             SELECT tmp
             INDEX on sitename+program+ENCNAME+last_name+first_name+serv_cat+DTOS(act_dt)tag repord
             
        ENDCASE   

        SELECT tmp
        COPY to crRptpath+"Activities_services.dbf"
        oApp.display_crystal_reports("activities_services_by_site.rpt")
...
*!*           LcFileName = "i:\ursver6\project\libs\display_reports.exe" 
*!*           LcAction = "open" 
*!*           Lcparms = "activities_services_by_site.rpt"
*!*           Lcdir = "i:\ursver6\airs_crreports\"
*!*           ShellExecute(0,LcAction,Lcfilename,lcparms,lcdir,1)
   ...
   Case lnStat = 3   &&Activities and Services -Summury
      ...
      Else
          ...
          * jss, 3/7/03, add program+serv_cat totals for topics here
          SELECT MyEnc.program ,;
            MyEnc.serv_cat ,;
            COUNT(*) AS top ;
          FROM MyEnc, MyServ3 ;
          ...
          SELECT sc.*,sc1.serv, sc2.totitems, sc4.top, sc3.totval, gcagencyname as agencyname,;
            oApp.gcversion as version, DTOC(oApp.gdverdate) as verdate, ;
            repcli.clitot as tot_clnts, repenc.enctot as tot_enctrs, repserv.servtot as tot_serv,;
            repitm.totitems as tot_items, reptop.toptot as tot_topics ;
          FROM sc ;
          LEFT OUTER join sc1 ON sc1.program+sc1.serv_cat = sc.program+sc.serv_cat ;
          LEFT OUTER JOIN sc2 ON sc2.program+sc2.serv_cat = sc.program+sc.serv_cat ;
          LEFT OUTER join sc3 ON sc3.program+sc3.serv_cat = sc.program+sc.serv_cat ;
          LEFT OUTER JOIN sc4 ON sc4.program+sc4.serv_cat = sc.program+sc.serv_cat ;
          into cursor temp readwrite

          REPLACE top WITH 0 FOR ISNULL(top)
          INDEX ON program+serv_cat tag progserv
  
          COPY TO CrRptpath+"activites_summry.dbf"

          oApp.display_crystal_reports("activities_summary_report.rpt")
...
*!*             LcFileName = "i:\ursver6\project\libs\display_reports.exe" 
*!*             LcAction = "open" 
*!*             Lcparms = "activities_summary_report.rpt"
*!*             Lcdir = "i:\ursver6\airs_crreports\"
*!*             ShellExecute(0,LcAction,Lcfilename,lcparms,lcdir,1)
       Endif 
EndCase
