...
IF !OpenAll("Creating Medicaid Claims.", "Open files ...")
	RETURN .F.
ENDIF
...
*** ********************************************************************************
IF !ComboThem(32,"Combining Encounters and Service Records...")
	RETURN .F.
ENDIF
...
************************************************************************************
FUNCTION OpenAll						
PARAMETER cMess1, cMess2

*	DO acttherm WITH cMess1, cMess2 IN thermo
	oThermo = createobject('thermobox', cMess1, cMess2)
	oThermo.show

	=OpenFile("claim_hd") 				&& claims header ***************************
	=OpenFile("claim_dt") 				&& claims details **************************
	=OpenFile("Es_bill","Enc_Ser")	 	&& Serv_id of Billed Encounters & Services *
...
*		DO deactthermo IN thermo
	...
	=OpenFile("enc_serv","spsrcs2")	 	&& encs/services procs & rates assignments *
										*** serv_cat & prog & site & rate_grp *
										*** & code & serv **************************
	IF !USED("staffcur")				&& cursor of staff *************************
*		oapp.build_staff_list()
		=OpenFile('staffcur')
	ENDIF
	SELECT staffcur
	SET ORDER TO worker_id
	
	=OpenFile("serv_loc", "code") 	&& service locations
	=OpenFile("ref_prov", "code") 	&& referring providers

	=OpenFile("ai_enc", .F., "the_enc") 	&& encounters ******************************
	=OpenFile("ai_serv", .F., "the_serv") && services ********************************
	...
	* Create a cursor of default procedure codes for services where one was not entered
	SELECT ;
		Serv_Cat, Prog, Site, Rate_Grp, enc_id, code, proc_code, modifier, location ;
	FROM ;
		enc_serv ;
	WHERE ;
		EMPTY(serv) AND ;
...
*	DO updtherm WITH nPercent, cMessage IN thermo
	...
	SCAN
		...
		CASE m.cat_serv <> Prov_cur.cat_serv AND Prov_cur.IsMedicaid
			RETURN FAIL("Check Provider Setup files. Category of " ;
				+ "Service are NOT all same for Provider# " + cProv_Num )
	...
	WHERE ;
		...
		!the_enc.not_bill AND ;
			(EXIST (SELECT * FROM do_rebill WHERE do_rebill.act_id = the_enc.act_id) OR ;
			 EXIST (SELECT * FROM do_reproc WHERE do_reproc.act_id = the_enc.act_id) ) AND;
		the_enc.act_dt >= the_site.effect_dt ;
	INTO CURSOR ;
		enc_site1 ;
...
*** ... Insured's Sex
*** ... Insured's DOB
*** ... Insured's Co-Payment type - leads to amount, percentage, or service amount
		...
		*** Get the rate code etc *****************************************
		IF EMPTY(all_serv.enc_rate)
			IF SEEK(	All_Serv.Serv_Cat + ;
						...
						All_Serv.Enc_Site + ;
						All_Serv.RG       + ;
						STR(All_Serv.Enc_id, 4, 0) + ;
						STR(All_Serv.Service_id, 4, 0), 'enc_serv')

*!*							All_Serv.Enc_Type + ;
*!*							All_Serv.Service, 'enc_serv')
...
*!*	BK 12/14/2005 - use new enc code
		IF EMPTY(all_serv.Proc_code) AND ;
				SEEK(Serv_Cat+ Enc_Prog+ Enc_Site+ RG+ STR(enc_id, 4, 0), 'def_proc')
*!*					SEEK(Serv_Cat+ Enc_Prog+ Enc_Site+ RG+ Enc_Type, 'def_proc')
...
*** SUM_SER_TM is total time of services *******************************************
	...
	DO WHILE !EOF("ToBill_cur")
		...
		SCAN WHILE ;
			...
			*** IF Non R_line Exist (Would not exist if !lNon_r) *******************
				*** When Actual services entered create non-R-Line (has proc code) *
				*** service lines in claim_dt, lNon_r does while !lNon_r does not **
				*** FORMA is actually !lNon_r **************************************
			*** *** Add Non R_lines ************************************************
			*** ELSE Non R_line does not actually Exist ****************************
				*** No actual services entered - so no recs with proc_data.proc_code
...
*			DO updtherm WITH 50 + nCount/RECCOUNT("ToBill_cur") * 50 IN thermo
			oThermo.refresh('', 50 + nCount/RECCOUNT("ToBill_cur") * 50)
			nCount = nCount + 1
...
PARAMETER cInvoice
	...
	VALUES ;
		...
		IIF(!EMPTY(ToBill_cur.diagnos1), ;
				oApp.Lookup("diagnos", "icd9code",ToBill_cur.diagnos1,"code","code"), ;
				diag_cur.icd9code), ;
		ToBill_cur.diagnos2, ;
				oApp.Lookup("diagnos","icd9code",ToBill_cur.diagnos2,	"code","code"), ;
		ToBill_cur.location, ;
		...
		staffcur.tax_id, ;
		ToBill_cur.ref_prov, ;
		oApp.Lookup("ref_prov", "id_no", ToBill_cur.ref_prov, "code", "code"), ;
		"0", "0", "30", ;
		gcWorker, m.dt, m.tm)
...
*	DO updtherm WITH nPercent, cMessage IN thermo
	...
	*** to Es_bill if the encounter/services is actually billed out ****************
	...
	*** Put Batch in old rebilled claim_dt recs, orig Inv/line in new rebill recs **
	...
	SCAN
		* Mark the new invoice with the invoice# and line of originating invoice
		IF SEEK(new_inv + new_rline,"claim_dt")
			...
			* Get the first invoice
			...
			ENDIF
		ELSE
			=Msg2User("MESSAGE","Can not find new claim. Call MIS Director.")
		ENDIF
		...
		* Mark originating invoice as "action performed" with current bill_id
		...
		ELSE
			=Msg2User("MESSAGE","Can not find old claim. Call MIS Director.")
		ENDIF
	ENDSCAN
...
************************************************************************************
*** Print Medicaid Claims Details report *******************************************
************************************************************************************
...
*** *** We are not involving service lines here, (we may not have to)
...
*	do acttherm with "Creating Adjust/Void Claims.", "Open files..." in thermo
...
*-*		claim_hd.claim_type = cClaimType AND ;
	...
	nTotal = _TALLY
	nCount = 0
*	do updtherm with 10, "Generate Adjust/Void Claims..." in thermo
...
*		do updtherm with 10 + nCount/nTotal * 90 in thermo
		oThermo.refresh('', 10 + nCount/nTotal * 90)
		nCount = nCount + 1
	ENDSCAN
...
PARAMETER cBillType
PRIVATE nunits
*** All Individual Services of a client on any date will be set up to be billed by time **
...
FUNCTION HIVBill
PARAMETER cBillType
*** Grouping is to get all HIV PC services of same encounter of a client ***********
...
FUNCTION ADHCBill
PARAMETER cBillType
*** Check For encounter time to be >= 180 mins. What about service time?
