...
* URS
***************************************************************
Parameters cBill_ID, cProv_ID, cProv2_ID, lReprocess, clClaimType
...
cDisk=" "
cClaimType="XN"   && Default to "837 Institutional - 5010" format

*!* Account for "Health Homes 837 Insititutional - 5010"
If Vartype(clClaimType)<>'L' And (Vartype(clClaimType)='C' And !Empty(clClaimType) And clClaimType='XY')
   cClaimType = "XY"
...
*************************************************************************************!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
**FOR NOW CHANGED TO TESTING!!!!!!!!!!!!!!
**cIndicate	= 'P'		&& System indicator T-Testing, P-Production
**cIndicate	= 'T'		&& System indicator T-Testing

=OpenFile('procpara')
Locate for procpara.claimtype=cClaimType

cIndicate=Iif(procpara.is_test=1, 'T', 'P')
cMedFile=''
cDisk=''
cIcd10DiagCode=Alltrim(procpara.icd10_code)
nUseIcd10Code=procpara.use_icd10_code
...
* 02/2012 Magnetic (AKA ETIN) is now 4 Char
...
cTax_ID = TRANS(cTax_ID, "@R 999999999")
...
**VT 06/23/2011 according to last meeting NPI not required
**VT 06/16/2011 AIRS 30
*!*	If Empty(cNPI)
*!*		oApp.msg2user("IMPORTANT",'* Required Information Missing *'+Chr(13)+"The NPI number must be entered!")
*!*		Select lv_claimlog_tmp
*!*		replace completed with 'No', Lcompl with .f.
*!*		
*!*		cLogId=lv_claimlog_tmp.log_id
*!*		Select claimlog
*!*		Locate for claimlog.log_id = cLogId
*!*		If Found()
...
**VT 07/19/2011 for testing purpose just 50 claims allowed added if else endif
If cIndicate='T'
		SELECT ;
			claim_hd.prov_id, ;
			claim_hd.prov_num, ;
      ...
      Select * Top 50 from t_claim Into Cursor claim_cur order by prov_id, prov2_id, loc_code, tc_id, invoice ReadWrite
      Select claim_cur
      Index ON ;
         prov_id + ;
         prov2_id + ;
...
*!*   		   If n=50
*!*   	   		Use in t_claim
*!*   	   		oApp.ReopenCur("claim_t", "claim_cur", .t.)
*!*   	   		Select claim_cur
*!*   	   					INDEX ON ;
*!*   								prov_id + ;
*!*   								prov2_id + ;
...
select distinct ;
	...
	iif(insstat.sig_onfile=1 , 'Y', 'I') as release_info;
from ;
...
SELECT distinct ;
	...
	address.city, ;
	IIF(address.zip <> '99999', PADR(Rtrim(address.zip), 9, '0'), '000010001') AS Zip, ;
	State.code AS state, ;
	client.phhome, ;
	client.phwork ;
FROM ;
	subs_cur, client, address, State ;
WHERE ;
	subs_cur.client_id = client.client_id and ;
	client.client_id = address.client_id and ;
	address.st = State.code ;
...
*----------------------------------------------------------------------------------------------------
*ISA*00*          *00*          *ZZ*000000000002589*ZZ*            25B*021211*1045*^*00501*000000001*0*T*:
*----------------------------------------------------------------------------------------------------
cOutputStr = 'ISA*'
cOutputStr = cOutputStr + '00*'										&& ISA01 -- Auth. Info Qualif.            (2)
cOutputStr = cOutputStr + '          *'							&& ISA02 -- Author. Information           (10)
cOutputStr = cOutputStr + '00*'										&& ISA03 -- Security Info Qualifier       (2)
cOutputStr = cOutputStr + '          *'							&& ISA04 -- Security Information          (10) 
cOutputStr = cOutputStr + 'ZZ*'										&& ISA05 -- Interchange ID qualifier      (2) 
cOutputStr = cOutputStr + PADR(cSub_ID, 15) + '*'				&& ISA06 -- Interchange Sender ID         (15) 
cOutputStr = cOutputStr + 'ZZ*'										&& ISA07 -- Interchange ID qualifier      (2)

*cOutputStr = cOutputStr + 'MMISNYDOH      *'					&& ISA08 -- Interchange Receiver ID
cOutputStr = cOutputStr + PADR(cIntRecID, 15) + '*'			&& ISA08 -- Interchange Receiver ID       (15)

cOutputStr = cOutputStr + cTranDate + '*'							&& ISA09 -- Interchange Date              (6)
cOutputStr = cOutputStr + cTranTime+ '*'							&& ISA10 -- Interchange Time              (4)

cOutputStr = cOutputStr + '^*'										&& ISA11 -- Repetition Seperator          (1)
cOutputStr = cOutputStr + '00501*'									&& ISA12 -- Interchange Control Version Number  (5)

cOutputStr = cOutputStr + PADL(cSerial, 9, '0') + '*'			&& ISA13 -- Interchange Control Number          (9)
cOutputStr = cOutputStr + '0*'										&& ISA14 -- Acknowledgement Requested           (1)
cOutputStr = cOutputStr + cIndicate + '*'							&& ISA15 -- Usage Indicator P/T - prod/test     (1)
cOutputStr = cOutputStr + ':'											&& ISA16 -- Component Element Separator         (1)
...
*----------------------------------------------------------------------------------------------------
*GS*HC*000000000002589*25B*20021211*1045*1*X*005010X223A2
*----------------------------------------------------------------------------------------------------
...
cOutputStr = cOutputStr + '005010X223A2'							&& GS08 -- Version Release Code
...
cOutputStr = cOutputStr +'005010X223A2' + '*'               &&ST03 -- Implementation Convention Reference
...
**VT 02/16/2011 Dev Tick 7860 V.U.8.6 
** 5.Transmition Set Identification-missing in version 5010 
*REF*87*004010X096A1
*----------------------------------------------------------------------------------------------------
...
**VT 06/16/2011 AIRS 30 take out else

IF !EMPTY(cNPI)
	cOutputStr = cOutputStr + 'XX*'										&& NM108 -- ID Code Qualifier
	cOutputStr = cOutputStr + Rtrim(cNPI)								&& NM109 -- NPI
EndIf
	
*!*	ELSE
*!*		cOutputStr = cOutputStr + '24*'										&& NM108 -- ID Code Qualifier
*!*		cOutputStr = cOutputStr + Rtrim(cTax_ID)									&& NM109 -- Provider Tax ID
*!*	ENDIF
...
cOutputStr = cOutputStr + PADR(Rtrim(m.zip), 9, '0')              && N403 -- Zip
...
* GOxford 2/29/2012
* - remove provider number and locator code from 2010AA
* - add locator code to 2010BB
* - all code in this section commented out 
*IF !lNPI_Req
*	cOutputStr = 'REF*'
...
*	ELSE
*		cOutputStr = cOutputStr + PADL(ALLTRIM(m.loc_code), 3, '0')
*	ENDIF
*	
*	nSegmentCount = nSegmentCount + 1
*	
...
* GOxford 2/29/2012 Locator code segment moved from 2010AA to here (2010BB) as per instructions during Health Homes test
* Keep the same logic as was used on 2010AA: send only if NPI required is not checked

IF !lNPI_Req
	cOutputStr = 'REF*'
	cOutputStr = cOutputStr + 'G2*'										&& REF01 Reference ID Qualifier
	cOutputStr = cOutputStr + PADL(ALLTRIM(m.loc_code), 3, '0')
	nSegmentCount = nSegmentCount + 1
	=WriteOutput(nFile, cOutputStr)
ENDIF
...
**VT 02/16/2011 Dev Tick 7860 V.U.8.6  not used
**cOutputStr = cOutputStr + 'Y*'														&& CLM06 -- Provider Signature on File
cOutputStr = cOutputStr + '*'														   && CLM06 -- Provider Signature on File
...
cOutputStr = cOutputStr + '*'														   && CLM18 -- Y/N cond. or resp. code - Explanation of Benefits Ind.
...
If !EMPTY(claim_cur.from_date) 
	cOutputStr=cOutputStr+CCYYMMDD(claim_cur.from_date)+'-'+CCYYMMDD(claim_cur.thru_date)  && DTP03 Service Dates (Range)
Else  
   cOutputStr=cOutputStr+CCYYMMDD(claim_cur.date)+'-'+CCYYMMDD(claim_cur.date)            && DTP03 Service Date
EndIf 

nSegmentCount = nSegmentCount + 1
=WriteOutput(nFile, cOutputStr)

**VT 02/16/2011 Dev Tick 7860 V.U.8.6  New   
**----------------------------------------------------------------------------------------------------
**Institutional Claim Code --------------------------------------------------------------------
**----------------------------------------------------------------------------------------------------
** CL1*1*1*30
**----------------------------------------------------------
**  see    Fed-837I.pdf   page    153        
** page 20 Fed-837 IA2 work with
           
  cOutputStr = 'CL1*'
       
  cOutputStr =  cOutputStr + '5*'       && CL101 Admission Type Code      HARD CODED to   5   - Information not available
  cOutputStr =  cOutputStr + '9*'        && CL102 Admission Source Code    Hard Coded 9 - Information not available
  cOutputStr =  cOutputStr + '30*'      && CL103 Patient Status Code      HARD CODED to  30 
...
*!*   Per ticket AIRS 216
*!*   If InList(Left(claim_cur.rate_code,4),'5223','1386')=(.f.)
...
*!*   Else
*!*      If Left(claim_cur.rate_code,4)='1386'
*!*         cDiag = '7999'
*!*      Else
*!*      	cDiag = '042'
*!*      EndIf 
...
*!* Per ticket AIRS 216; go back to 042
*!* If !InList(Left(claim_cur.rate_code,4),'5223','1880','1881','1386','1387')
*!* If claim_cur.rate_code <> '5223'

cDiagIndic='BK:'

*!*   If (cIndicate=('T') And !Empty(cIcd10DiagCode)) Or ;
*!*      (cIndicate=('P') And !Empty(cIcd10DiagCode) And nUseIcd10Code=(1) And claim_cur.from_date>={10/01/2015})

If (cIndicate=('T') And !Empty(cIcd10DiagCode)) Or ;
   (!Empty(cIcd10DiagCode) And claim_cur.date>={10/01/2015})

   cDiagIndic='ABK:'
   cDiag=cIcd10DiagCode
Else
   cDiagIndic='BK:'
...
cOutputStr = 'HI*'
cOutputStr = cOutputStr + cDiagIndic                     && HI01-1 -- Diagnosis Type Code (BK=primary)
...
**cOutputStr = cOutputStr + Rtrim(Str(Val(rate_code))) 			      && HI01-5 -- Value Code Associated Amount
...
*!* If !InList(Left(claim_cur.rate_code,4),'5223','1386') AND !EMPTY(claim_cur.bill_phys) AND SEEK(claim_cur.bill_phys, 'staff_cur')
If !InList(Left(claim_cur.rate_code,4),'5223','1880','1881','1386','1387') AND !EMPTY(claim_cur.bill_phys) AND SEEK(claim_cur.bill_phys, 'staff_cur')
	*----------------------------------------------------------------------------------------------------
...
*!*		ENDIF
	*** - end of 1/11/2007
	...
	cOutputStr = 'REF*'
	cOutputStr = cOutputStr + IIF(staff_cur.medicaid, 'G2', '0B') + '*'				&& REF01 Reference ID Qualifier
...
**VT 02/17/2011 Dev Tick 7860 V.U.8.6  New   Not REQUIRED
**----------------------------------------------------------------------------------------------------
** 39.Other Insurance Coverage Information --------------------------------------------------------------------
**----------------------------------------------------------------------------------------------------
**  OI***Y***A

*!*	cOutputStr = 'OI*'
*!*	cOutputStr = cOutputStr + '*' 														      && OI01 Claim Filing Indicator (N/A)
*!*	cOutputStr = cOutputStr + '*' 														      && OI02 Claim Submission Reason (N/A)
*!*	cOutputStr = cOutputStr + 'Y*' 													         && OI03 Assignments of Benefits Indicator
*!*	cOutputStr = cOutputStr + '*' 														      && OI04 Patient Signature Source Code
*!*	cOutputStr = cOutputStr + '*' 													         &&OI05 Provider Agreement Code (N/A)
*!*	cOutputStr = cOutputStr + subs_cur.release_info
...
** If !InList(Left(claim_cur.rate_code,4),'5223','1386') AND !EMPTY(claim_cur.proc_code)

If !InList(Left(claim_cur.rate_code,4),'5223','1880','1881','1386','1387') AND !EMPTY(claim_cur.proc_code)
