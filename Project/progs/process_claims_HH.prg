...
Select ;
   tc_id, ;
   start_dt, ;
   Iif(Empty(end_dt), dEndDate, end_dt) As end_dt ;
From ai_prog ;
Where program=clProgram ;
Into Cursor _curEnrollments;
Order By ;
   tc_id, ;
   start_dt Desc

Select _curEnrollments
Go Top
Scan
   If Between(dStartDate,start_dt,end_dt)
      m.tc_id=_curEnrollments.tc_id
      If Seek(tc_id+Dtos(dStartDate),'hh_bill_log','tcid_date')=(.f.)
         chhlogid=GetNextID("HH_BILL")
         Insert into hh_bill_log (hh_bill_log_id, tc_id, act_dt, ai_prog_id, rate_grp, not_bill, user_id, dt, tm);
                          Values (chhlogid, m.tc_id, start_dt, clProgram, clrate_grp, .f., gcworker, dt, tm)
      EndIf
   EndIf
EndScan 
...
*	ENDIF
	...
	* =OpenFile("serv_loc", "code") 	&& service_id locations
	...
	=OpenFile("hh_bill_log", .F., "the_enc") 	&& encounters ******************************
	*** ****************************************************************************
...
************************************************************************************
Function PreSelEncs (nPercent, cMessage, lNewBill, lReBill)
Private iicond
...
Where ;
	...
	!hh_bill.Rebilled AND ;
	hh_bill.invoice=claim_hd.invoice AND ;
	hh_bill.r_line=claim_dt.line_no ;
Into Cursor ;
	do_rebill
...
Where ;
	...
	!hh_bill.Rebilled AND ;
	hh_bill.invoice=claim_hd.invoice AND ;
	hh_bill.r_line=claim_dt.line_no AND ;
	hh_bill.Bill_ID=cBill_ID ;
Into Cursor ;
	do_reproc
...
Select ;
	...
	Space(04) As beg_tm, ;
	Space(02) As beg_am, ;
	cDef_Phys As bill_phys, ;
	Space(05) As ref_prov, ;
	{} As date_compl, ;
	Space(05) As end_tm, ;
	Space(02) As end_am, ;
	0 As enc_id, ;
	Space(02) As enc_modif, ;
	lcProgram As enc_prog, ;
	Space(05) As serv_cat, ;
	Space(05) As enc_site, ;
	the_enc.tc_id, ;
	Space(05) As rate_grp, ;
	Space(05) As enc_rate, ;
	Space(05) As enc_proc, ;
	Space(05) As enc_prov, ;
	Space(05) As act_loc, ;
	Space(05) As bill_place, ;
	Space(05) As diagnos1, ;
	Space(05) As diagnos2, ;
	Space(05) As home_site, ;
	{} As effsite_dt ;
From  ;
	the_enc ;
Where ;
	...
	Not Exist (Select * ;
              From hh_bill ;
              Where ;
					 hh_bill.hh_bill_id = the_enc.hh_bill_log_id AND ;
					 !hh_bill.ReBilled ) ;
Into Cursor t_site1
...
************************************************************************************
Function ClearClaim (nPercent, cMessage)
