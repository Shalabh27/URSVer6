...
**VT 09/02/2010 Dev Tick 7386 add case_cat
SELECT  ;
	cli_cur.*, ;
...
INTO CURSOR ;
	EncCli
*  Space(30) as prg_descript, ; 

**AND ai_enc.program   = lcprog;   VT 01/07/2008
...
Use in t_stat
***************************************************************************
**VT 09/02/2010 Dev Tick 7386 remove SPACE(2) AS how_prov, SPACE(30)  AS  prov, ;  add serv_date
...
**VT 09/02/2010 Dev Tick 7386 add serv_date  ai_serv.how_prov,; remove SPACE(30)  AS  prov, ;
...
*!*            IF FOUND()
*!*               REPL MyServ.serv WITH service.descript
*!*            ENDIF
      SELECT serv_list
...
* jss, 3/7/03, add toptot for topic count by program
   ...
   Case lnStat = 1    &&Activities and Services
    ...
    Case lnStat = 2   &&Activities and Services by Site
         ...
         Else
                 ...
                 DO CASE
                     CASE nOrder = 1
                          **cOrd = "sitename+program+last_name+first_name+serv_cat+DTOS(act_dt)"
                          cOrd = "sitename+program+Upper(Alltrim(last_name)+Alltrim(first_name))+serv_cat+DTOS(act_dt)"
                     CASE nOrder = 2
                          **cOrd = "sitename+program+ENCWORK+last_name+first_name+serv_cat+DTOS(act_dt)"
                          cOrd = "sitename+program+ENCWORK+Upper(Alltrim(last_name)+Alltrim(first_name))+serv_cat+DTOS(act_dt)"
                     CASE nOrder = 3
                          **cOrd = "sitename+program+ENCNAME+last_name+first_name+serv_cat+DTOS(act_dt)"
